...
import { Award, Briefcase, DollarSign, Loader2, MapPin, Search, Sparkles, Star, Stethoscope, User } from "lucide-react";
import Image from "next/image";
import Link from "next/link";
import { useEffect, useState } from "react";
import { toast } from "react-hot-toast";
import { Badge } from "../ui/badge";
import { getAIDoctorSuggestion } from "@/services/ai/ai";
...
export default function AISearchDialog({ initialSymptoms = "", externalOpen, onOpenChange, onSearchComplete }: AISearchDialogProps = {}) {
  ...
  useEffect(() => {
    if (open && triggerSearch && symptoms && symptoms.trim().length >= 5 && !hasAutoSearched) {
    ...
    <Dialog open={open} onOpenChange={handleDialogOpenChange}>
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <DialogDescription>Describe your symptoms to find the right doctor</DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="space-y-4 mt-4">
          ...
          <Button onClick={handleSearch} disabled={isLoading || symptoms.trim().length < 5} className="w-full" size="lg">
            {isLoading ? (
              <>
          ...
          {showSuggestions && suggestedDoctors.length > 0 && (
            <div className="space-y-4 p-4 bg-linear-to-br from-primary/5 to-white rounded-lg border-2 border-primary/20">
                  ...
                  <div key={doctor.id || index} className="p-4 bg-white rounded-lg border border-primary/20 hover:shadow-md transition-shadow">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 space-y-2">
                        ...
                        <div className="shrink-0">
                          {doctor.profilePhoto ? (
                            <Image src={doctor.profilePhoto} alt={doctor.name} width={64} height={64} className="w-16 h-16 rounded-full object-cover border-2 border-primary/30" />
                          ) : (
              ...
              <div className="pt-2 border-t border-blue-200">
                <p className="text-xs text-center text-muted-foreground">⚠️ AI suggestions are for guidance only. Please consult a medical professional for accurate diagnosis.</p>
              </div>
            </div>
          )}
          ...
          {showSuggestions && suggestedDoctors.length === 0 && (
            <div className="p-6 bg-amber-50 rounded-lg border-2 border-amber-200 text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1">Try describing your symptoms differently.</p>
            </div>
          )}
        </div>
      </DialogContent>
